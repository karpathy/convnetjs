...
(function(global) {
  ...
  // The LpPoolLayer does general L-p pooling with trainable p
  // idea is from a paper at NIPS 2013 workshop 
  // Learned-norm pooling for deep neural networks
  // by Caglar Gulcehre et al.
  var LpPoolLayer = function(opt) {
    ...
    this.out_sx = Math.floor(this.in_sx / this.stride); // compute size of output volume
    this.out_sy = Math.floor(this.in_sy / this.stride);
    this.layer_type = 'lppool';
    this.ps = new Vol(this.out_sx, this.out_sy, this.out_depth, 2.0); // init to l2 norm pooling
    this.dps = zeros(this.out_sx*this.out_sy*this.out_depth);
  }
  LpPoolLayer.prototype = {
    forward: function(V, is_training) {
      this.in_act = V;
      ...
      for(var d=0;d<this.out_depth;d++) {
        ...
        for(var x=0;x<V.sx;x+=this.stride,ax++) {
          ...
          for(var y=0;y<V.sy;y+=this.stride,ay++) {
            ...
            var p = this.ps[n];
            var p1 = p-1;
            var a = 0.0; // hopefully small enough ;\
            var a1 = 0.0;
            for(var fx=0;fx<this.sx;fx++) {
              for(var fy=0;fy<this.sy;fy++) {
                var oy = y+fy-hy;
                var ox = x+fx-hx;
                if(oy>=0 && oy<V.sy && ox>=0 && ox<V.sx) {
                  var v = Math.abs(V.get(ox, oy, d));
                  a += Math.pow(v, p);
                  a1 += Math.pow(v, p1);
                }
              }
            }
            this.dps[n] = Math.pow(a, 1.0/p - 1.0) * a1;
            A.set(ax, ay, d, Math.pow(a, 1.0/p));
            n++;
          }
        }
      }
      ...
      for(var d=0;d<this.out_depth;d++) {
        ...
        for(var x=0;x<V.sx;x+=this.stride,ax++) {
          ...
          for(var y=0;y<V.sy;y+=this.stride,ay++) {
            ...
            var gp = this.dps[n];
            this.ps.add_grad(x,y,d,gp); // we computed the gradient in forward pass
            V.add_grad(x,y,d,gp*chain_grad);
            n++;
          }
        }
      }
