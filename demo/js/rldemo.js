...
var World = function () {
    ...
    this.items = []
    for (var k = 0; k < 30; k++) {
        var x = convnetjs.util.randf(20, this.W - 20);
        var y = convnetjs.util.randf(20, this.H - 20);
        var t = convnetjs.util.randi(1, 3); // food or poison (1 and 2)
...
World.prototype = {
    ...
    tick: function () {
        ...
        for (var i = 0, n = this.items.length; i < n; i++) {
            ...
            if (it.age > 5000 && this.clock % 100 === 0 && convnetjs.util.randf(0, 1) < 0.1) {
        ...
        if (this.items.length < 30 && this.clock % 10 === 0 && convnetjs.util.randf(0, 1) < 0.25) {
            var newitx = convnetjs.util.randf(20, this.W - 20);
            var newity = convnetjs.util.randf(20, this.H - 20);
            var newitt = convnetjs.util.randi(1, 3); // food or poison (1 and 2)
...
var reward_graph = new convnetjs.cnnvis.Graph();
function draw_stats() {
...
function reload() {
    ...
    reward_graph = new convnetjs.cnnvis.Graph(); // reinit
}
