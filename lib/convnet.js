var convnetjs = (function(exports){
  ...
  var Vol = function(sx, sy, depth, c) {
    ...
    if(typeof c === 'undefined') {
      ...
      this.w = [];
      this.dw = [];
      var scale = Math.sqrt(3.0/(sx*sy*depth));
  ...
  */
  ...
  SoftmaxLayer.prototype = {
    ...
    getParamsAndGrads: function() {
      // naughty hack. Sometimes we want higher learning rate for softmax only
      var gmul =1.0; 
      if(this.gmul) gmul = this.gmul; 

      var response = [];
      for(var i=0;i<this.out_depth;i++) {
        response.push({params: this.filters[i].w, grads: this.filters[i].dw, gmul:gmul, decay: true});
      }
      response.push({params: this.biases.w, grads: this.biases.dw, gmul:gmul, decay:false});
      return response;
    },
    toJSON: function() {
      var json = {};
    ...
    train: function(x, y) {
      ...
      if(this.k % this.batch_size === 0) {
        ...
        for(var i=0;i<pglist.length;i++) {
          ...
          // gmul is a naughty hack that can scale
          // learning rate for some parameters.
          var gmul = 1.0;
          if(pg.gmul) {gmul = pg.gmul;}
          ...
          for(var j=0;j<p.length;j++) {
            ...
            if(this.momentum > 0.0) {
              ...
              var dir = -gmul*this.learning_rate * (decay * p[j] +  g[j]) / this.batch_size;
